...
const IncorrectArchiveScreen = ({ setCurrentScreen }) => {
  const [incorrectAnswersList, setIncorrectAnswersList] = useState([]);
  const [searchText, setSearchText] = useState("");
  const { restoreScrollPosition } = useScrollPosition(
    "incorrectArchiveScrollPosition",
  );
  ...
  const handleScroll = () => {
    ...
    scrollTimeoutRef.current = setTimeout(() => {
      ...
      if (Math.abs(currentScrollY - lastScrollYRef.current) > 100) {
        ...
        console.log(
          `📜 РУЧНОЕ сохранение при скролле (Неправильные): ${currentScrollY}px`,
        );
      }
    }, 500);
  };
  ...
  const handleBackClick = () => {
    const currentScrollY = window.scrollY;
    console.log(
      `🔙 КЛИК НАЗАД (Неправильные): сохранение позиции ${currentScrollY}px`,
    );
    localStorage.setItem(
      "incorrectArchiveScrollPosition",
      currentScrollY.toString(),
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      const currentScrollY = window.scrollY;
      console.log(
        `📄 СОХРАНЕНИЕ при закрытии (Неправильные): ${currentScrollY}px`,
      );
      localStorage.setItem(
        "incorrectArchiveScrollPosition",
        currentScrollY.toString(),
  ...
  useEffect(() => {
    if (searchText) {
      window.scrollTo(0, 0);
      localStorage.setItem("incorrectArchiveScrollPosition", "0");
  ...
  useEffect(() => {
    ...
    if (savedIncorrect) {
      setIncorrectAnswersList(JSON.parse(savedIncorrect));
    }
  }, []);
  ...
  // Фильтрация вопросов
  const filteredQuestions = incorrectAnswersList.filter((questionId) => {
  ...
  const removeFromArchive = (questionId) => {
    const updated = incorrectAnswersList.filter((id) => id !== questionId);
    setIncorrectAnswersList(updated);
    localStorage.setItem("incorrectAnswersList", JSON.stringify(updated));
    ...
    delete userAnswersMain[questionId];
    delete userAnswersFast[questionId];

    localStorage.setItem("userAnswers", JSON.stringify(userAnswersMain));
    localStorage.setItem("userAnswersFast", JSON.stringify(userAnswersFast));
  };

  // Очистка архива
  const clearArchive = () => {
    const confirmClear = window.confirm("Очистить архив неправильных ответов?");
    if (confirmClear) {
      setIncorrectAnswersList([]);
      localStorage.setItem("incorrectAnswersList", JSON.stringify([]));
    }
  };
  ...
  const downloadArchive = () => {
    ...
    const header = `Архив: Неправильные ответы / Не знаю\nДата: ${new Date().toLocaleString("ru-RU")}\nВсего: ${incorrectAnswersList.length}\n\n`;

    const lines = incorrectAnswersList.map((questionId, index) => {
      ...
      if (userAnswerIndex === null) {
        ...
        chosen = `${userAnswerIndex + 1}) ${q.answers[userAnswerIndex]?.text || "—"}`;
      }
    ...
    downloadTextFile(content, "incorrect_answers.txt");
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {incorrectAnswersList.length > 0 && `(${incorrectAnswersList.length})`}
      </h1>
      ...
      {filteredQuestions.length === 0 ? (
        <p style={{ textAlign: "center", marginTop: "50px", color: "#666" }}>
          {incorrectAnswersList.length === 0
            ? "Пока нет неправильных ответов в архиве"
            : "Вопросы не найдены"}
        </p>
      ) : (
            ...
            <div key={questionId} style={cardStyle}>
              ...
              <ul style={{ listStyle: "none", padding: 0, marginTop: 15 }}>
                {q.answers.map((a, j) => {
                  ...
                  if (a.correct) {
                    bgColor = "#4caf50"; // Правильный ответ
                  } else if (userAnswerIndex === j) {
                    bgColor = "#f44336"; // Ответ пользователя, который неправильный
                  }
...
export default IncorrectArchiveScreen;
