...
const AllQuestionsArchiveScreen = ({ setCurrentScreen }) => {
  const [searchText, setSearchText] = useState("");
  const { restoreScrollPosition } = useScrollPosition(
    "allQuestionsArchiveScrollPosition",
  );
  ...
  const handleScroll = () => {
    ...
    scrollTimeoutRef.current = setTimeout(() => {
      ...
      if (Math.abs(currentScrollY - lastScrollYRef.current) > 100) {
        ...
        console.log(
          `📜 РУЧНОЕ сохранение при скролле (Все вопросы): ${currentScrollY}px`,
        );
      }
    }, 500);
  };
  ...
  const handleBackClick = () => {
    const currentScrollY = window.scrollY;
    console.log(
      `🔙 КЛИК НАЗАД (Все вопросы): сохранение позиции ${currentScrollY}px`,
    );
    localStorage.setItem(
      "allQuestionsArchiveScrollPosition",
      currentScrollY.toString(),
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      const currentScrollY = window.scrollY;
      console.log(
        `📄 СОХРАНЕНИЕ при закрытии (Все вопросы): ${currentScrollY}px`,
      );
      localStorage.setItem(
        "allQuestionsArchiveScrollPosition",
        currentScrollY.toString(),
  ...
  useEffect(() => {
    if (searchText) {
      window.scrollTo(0, 0);
      localStorage.setItem("allQuestionsArchiveScrollPosition", "0");
      lastScrollYRef.current = 0;
    }
  }, [searchText]);

  // Фильтрация всех вопросов
  const filteredQuestions = questionsData
    ...
    .filter((q) => {
      ...
      if (searchText.match(/^\d+$/) && String(q.id + 1).includes(searchText)) {
        return true;
      }
...
export default AllQuestionsArchiveScreen;
