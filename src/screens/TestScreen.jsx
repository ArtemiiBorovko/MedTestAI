...
function TestScreen({ setCurrentScreen }) {
  ...
  const photosCheckRef = useRef(false);
  ...
  // НАЙТИ ПЕРВЫЙ НЕОТВЕЧЕННЫЙ ВОПРОС В ОСНОВНОМ ТЕСТЕ
  const findFirstUnansweredQuestion = () => {
    ...
    return questionsData.length - 1; // Все вопросы отвечены
  };

  // НАЙТИ ГРАНИЦУ ПРОГРЕССА - ПОСЛЕДНИЙ ОТВЕЧЕННЫЙ ПЕРЕД ПЕРВЫМ НЕОТВЕЧЕННЫМ
  ...
  const findNextUnansweredQuestion = (startFrom = 0) => {
    for (let i = startFrom; i < questionsData.length; i++) {
      if (userAnswers[i] === undefined) {
        return i;
      }
    }
    return questionsData.length; // Все вопросы отвечены
  };
  ...
  // ИНИЦИАЛИЗАЦИЯ: установить текущий вопрос на границу прогресса
  useEffect(() => {
    if (!initializedRef.current) {
      const progressBoundary = findProgressBoundary();
      //console.log("Initializing TestScreen at boundary:", progressBoundary);
      setCurrentQuestion(progressBoundary);
      initializedRef.current = true;
    }

    const savedCorrect = localStorage.getItem("correctAnswersList");
    const savedIncorrect = localStorage.getItem("incorrectAnswersList");

    if (savedCorrect) setCorrectAnswersList(JSON.parse(savedCorrect));
    if (savedIncorrect) setIncorrectAnswersList(JSON.parse(savedIncorrect));

    setReturnToIndex(null);
    setIsPreviewMode(false);
    document.title = "Medical Test";
  }, [userAnswers]);

  // Preload фото
  useEffect(() => {
    if (photosCheckRef.current) return;
    photosCheckRef.current = true;
    const exists = [];
    let checked = 0;
    if (!photoQuestionNumbers || photoQuestionNumbers.length === 0) {
      setExistingPhotos([]);
      return;
    }
    photoQuestionNumbers.forEach((num) => {
      const img = new Image();
      img.onload = () => {
        exists.push(num);
        checked++;
        if (checked === photoQuestionNumbers.length) {
          exists.sort((a, b) => a - b);
          setExistingPhotos(exists);
        }
      };
      img.onerror = () => {
        checked++;
  ...
  useEffect(() => {
    if (currentQuestion < questionsData.length) {
      ...
      questionsData[currentQuestion].answers.forEach((_, idx) => {
        setTimeout(() => {
  ...
  // Предзагрузка картинок с обработкой ошибок
  useEffect(() => {
    const nextNum = currentQuestion + 2;
    const nextSrc = `/images/${nextNum}.png`;
    preloadImage(nextSrc).catch(() => {
      //  console.log(`Image not found: ${nextSrc}`);
    });
  }, [currentQuestion]);

  // Helper функции для архивов
  const saveCorrectToStorage = (list) => {
    localStorage.setItem("correctAnswersList", JSON.stringify(list));
  };

  const saveIncorrectToStorage = (list) => {
    localStorage.setItem("incorrectAnswersList", JSON.stringify(list));
  };

  const makeArchiveText = (list, type) => {
    const userAnswers = getUserAnswersByType("main");
    const header = `Архив: ${type === "correct" ? "Правильные" : "Неправильные/Не знаю"}\nДата: ${new Date().toLocaleString("ru-RU")}\nВсего: ${list.length}\n\n`;
    const lines = list.map((qIdx, i) => {
      ...
      const chosen =
        ua === null
          ...
          : ua === undefined
            ? "—"
            : `${ua + 1}) ${q.answers[ua] ? q.answers[ua].text : "—"}`;
      const correctText = q.answers.find((a) => a.correct)?.text || "—";
      return `${i + 1}. #${qIdx + 1} ${q.question.replace(/^\d+\.\s*/, "")}\n   Ваш ответ: ${chosen}\n   Правильный: ${correctText}\n`;
    });
    return header + lines.join("\n");
  };

  const downloadTextFileWithBom = (text, filename) => {
    try {
  ...
  const handleAnswer = (index) => {
    ...
    const correct = questionsData[currentQuestion].answers[index].correct;

    // Обновляем архивы
    const { newCorrectList, newIncorrectList } = updateAnswerArchives(
      currentQuestion,
      correct,
      index,
      "main",
    ...
    setCorrectAnswersList(newCorrectList);
    setIncorrectAnswersList(newIncorrectList);

    // Обновляем единую статистику
    if (correct) {
    ...
    updateUserAnswer(currentQuestion, index);
  ...
  const handleUnknown = () => {
    ...
    // Обновляем userAnswers
    updateUserAnswer(currentQuestion, null);
  ...
  // Следующий вопрос ведет к следующему неотвеченному
  const nextQuestion = () => {
    setAnimateIdx(null);
    setShowAnswer(false);
    setAnswerAppear(Array(4).fill(false));
    ...
    setTimeout(() => {
      ...
      if (nextUnanswered >= questionsData.length) {
        setCurrentQuestion(questionsData.length);
      } else {
  ...
  const handleSearch = () => {
    ...
    if (!Number.isNaN(num) && num >= 1 && num <= questionsData.length) {
    ...
    const foundIdx = questionsData.findIndex((q) =>
  ...
  const prevSelected = userAnswers[currentQuestion];
      ...
      {currentQuestion < totalQuestions ? (
        ...
        >
          ...
          >
            ...
            >
              {currentQuestion + 1}.{" "}
              {questionsData[currentQuestion].question.replace(/^\d+\.\s*/, "")}
            </h2>
            <div
              style={{
                marginTop: "-5px",
          ...
          {/* Картинка вопроса с улучшенной обработкой ошибок */}
            ...
            <div style={{ marginTop: 12 }}>
              <img
                ...
                onError={(e) => {
                  ...
                  //console.log(
                  //  `Image not found: /images/${currentQuestion + 1}.png`,
                  //);
                }}
              />
            </div>
          ...
          <ul style={{ listStyle: "none", padding: 0, marginTop: 15 }}>
            {questionsData[currentQuestion].answers.map((ans, idx) => {
              let bgColor = "#2c2c2c";
              const canAnswer =
                !isPreviewMode && userAnswers[currentQuestion] === undefined;
        ...
        <>
          <h1 style={{ textAlign: "center", marginTop: 50 }}>Вы прошли тест</h1>
